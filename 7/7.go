...
//	res_a := 0
	options := map[int]bool{0: true, 1: true, 2: true, 3: true, 4: true}


	for a, _ := range options {
		delete(options, a)
		res_a := sim(codes, a, 0)
		for b, _ := range options {
			delete(options, b)
			res_b := sim(codes, b, res_a)
			for c, _ := range options {
				delete(options, c)
				res_c := sim(codes, c, res_b)
				for d, _ := range options {
					delete(options, d)
					res_d := sim(codes, d, res_c)
					for e, _ := range options {
